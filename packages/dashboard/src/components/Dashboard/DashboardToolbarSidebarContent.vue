<template>
  <div :class="bem('sidebar')">
    <Card v-if="apiModeOptions?.length" size="small" title="数据源模式" :class="bem('card')">
    ...
    <Card size="small" title="操作" :class="bem('card')">
      <Flex gap="8" wrap>
        <Button size="small" type="ghost" :disabled="isBooting || isReadOnly" @click="emit('create-group')">创建面板组</Button>
      </Flex>
      <div :class="bem('divider')"></div>
      <Flex gap="8" wrap>
        <Button size="small" type="ghost" :icon="h(FileTextOutlined)" :disabled="isBooting" @click="emit('view-json')">查看</Button>
        <Button size="small" type="ghost" :icon="h(UploadOutlined)" :disabled="isBooting || isReadOnly" @click="emit('import-json')">导入</Button>
        <Button size="small" type="ghost" :icon="h(DownloadOutlined)" :disabled="isBooting" @click="emit('export-json')">导出</Button>
      </Flex>
    ...
    <Card size="small" title="视图与时间" :class="bem('card')">
      ...
      <div :class="bem('field')">
        ...
        <TimeRangePicker :value="draftTimeRange" :disabled="isBooting" @update:value="(value: string) => emit('update:draft-time-range', value)" />
      </div>
      <div :class="bem('field')">
...
<script setup lang="ts">
  import { h } from 'vue';
  import { Button, Card, Flex, Input, Segmented, Select, TimeRangePicker } from '@grafana-fast/component';
