 ...
 */
import type { DashboardSessionKey, QueryContext, QueryExecuteDTO, QueryResult } from '@grafana-fast/types';

/**
 * QueryService（契约层）
 *
 * 核心约束：
 * - UI/存储层使用 CanonicalQuery（落库/导入导出）
 * - 执行层使用 QueryExecuteDTO（发往后端执行接口的固定入参）
...
export interface QueryService {
   ...
   * - 返回 QueryResult[] 与输入 queries 对齐（必须包含 queryId 以便 UI 对齐）
   */
  executeQueries: (queries: QueryExecuteDTO[], context: QueryContext, options?: ExecuteQueriesOptions) => Promise<QueryResult[]>;
