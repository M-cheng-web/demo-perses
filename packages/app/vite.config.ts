 ...
 * - 通过 `pnpm dev:prod`（Vite mode=production）切换为消费各包的 dist 产物（更接近发布后的集成形态）
 */
...
export default defineConfig(({ mode }) => {
  // 约定：
  // - mode=development：联调模式（消费 workspace 源码）
  // - mode=production：集成验证模式（消费各包 dist 产物）
  const useDist = mode === 'production';

  return {
    base: './',
    // 说明：在 pnpm + workspace 下，vite/plugin-vue 可能出现“重复 vite 类型”导致的 TS 类型不兼容。
    // 这不影响实际运行与构建产物，因此这里做一个窄范围的类型收敛，避免阻断启动/构建。
    plugins: [vue() as any],
    resolve: {
      alias: {
        '@': resolveFromRoot('./src'),
