  ...
  <Teleport to="body">
    ...
    >
      ...
      <div class="tooltip-content">
        ...
        <div v-if="displayedSeries.length === 0" class="tooltip-empty">
          暂无数据
        </div>
      </div>
    </div>
...
const findNearbySeries = () => {
  ...
  try {
    ...
    const pointInGrid = props.chartInstance.convertFromPixel(
      { seriesIndex: 0 },
      pointInPixel
    );
    ...
    props.data.forEach((timeSeries, index) => {
      ...
      timeSeries.values.forEach(([timestamp, value]) => {
        ...
        if (distance < minDistance && distance < 60000) { // 60秒容差
      ...
      if (closestPoint) {
        ...
        const color = seriesOption?.lineStyle?.color || 
